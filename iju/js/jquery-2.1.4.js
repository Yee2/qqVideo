				...
				function(window, noGlobal) {
					...
					jQuery.extend({
						...
						isPlainObject : function(obj) {
							// Not plain objects:
							// - Any object or value whose internal [[Class]]
							// property is not "[object Object]"
							// - DOM nodes
							// - window
							...
							// If the function hasn't returned already, we're
							// confident that
						...
						globalEval : function(code) {
							...
							if (code) {
								// If the code includes a valid, prologue
								// position
								// strict mode pragma, execute code by injecting
								// a
								...
								if (code.indexOf("use strict") === 1) {
									...
									// Otherwise, avoid the DOM node creation,
									// insertion
									// and removal by using an indirect global
									// eval
									indirect(code);
								}
							}
					...
					(function(window) {
						...
						setDocument = Sizzle.setDocument = function(node) {
							...
							if ((support.qsa = rnative.test(doc.querySelectorAll))) {
								...
								assert(function(div) {
									...
									docElem.appendChild(div).innerHTML = "<a id='" + expando + "'></a>" + "<select id='" + expando + "-\f]' msallowcapture=''>"
							...
							sortOrder = hasCompare ? function(a, b) {
								...
								if (!aup || !bup) {
									...
									// If the nodes are siblings, we can do a
									// quick check
								} else if (aup === bup) {
								...
								"ATTR" : function(name, operator, check) {
									return function(elem) {
										...
										return operator === "=" ? result === check : operator === "!=" ? result !== check : operator === "^=" ? check
												...
												&& result.slice(-check.length) === check : operator === "~=" ? (" " + result.replace(rwhitespace, " ") + " ")
												.indexOf(check) > -1 : operator === "|=" ? result === check || result.slice(0, check.length + 1) === check + "-" : false;
									};
								},
						...
						function addCombinator(matcher, combinator, base) {
							...
							function(elem, context, xml) {
								...
								// We can't set arbitrary data on XML nodes, so
								// they don't benefit from dir caching
								if (xml) {
									while ((elem = elem[dir])) {
										if (elem.nodeType === 1 || checkNonElements) {
							...
							for (; i < len; i++) {
								if ((matcher = Expr.relative[tokens[i].type])) {
									...
									if (matcher[expando]) {
										...
										// descendant combinator, insert an
					...
					jQuery.filter = function(expr, elems, not) {
						...
						return elems.length === 1 && elem.nodeType === 1 ? jQuery.find.matchesSelector(elem, expr) ? [ elem ] : [] : jQuery.find.matches(expr, jQuery
					...
					jQuery.fn.extend({
						...
						index : function(elem) {
							...
							// If it receives a jQuery object, the first element
							// is used
					...
					jQuery.extend({
						...
						Deferred : function(func) {
							...
							[ "resolve", "done", jQuery.Callbacks("once memory"), "resolved" ], [ "reject", "fail", jQuery.Callbacks("once memory"), "rejected" ],
							...
							jQuery.each(tuples, function(i, tuple) {
								...
								if (stateString) {
									list.add(function() {
										...
										// [ reject_list | resolve_list
										// ].disable; progress_list.lock
						...
						when : function(subordinate /* , ..., subordinateN */) {
							...
							// Update function for both resolve and progress
							// values
							...
							// Add listeners to Deferred subordinates; treat
							// others as resolved
							if (length > 1) {
							...
							// If we're not waiting on anything, resolve the
							// master
							if (!remaining) {
						...
						if (data === undefined && elem.nodeType === 1) {
							...
							if (typeof data === "string") {
								try {
									...
									// Only convert to a number if it doesn't
									// change the string
					...
					jQuery.fn.extend({
						data : function(key, value) {
							...
							return access(this, function(value) {
								...
								// The calling jQuery object (element matches)
								// is not empty
								// (and therefore has an element appears at
								// this[ 0 ]) and the
								// `value` parameter was not undefined. An empty
								// jQuery object
								...
								// Set the data...
								this.each(function() {
									// First, attempt to store a copy or
									// reference of any
									// data that might've been store with a
									// camelCased key.
									var data = data_user.get(this, camelKey);

									// For HTML5 data-* attribute interop, we
									// have to
									...
									// _actually_
									// have dashes, we need to also store a copy
									// of that
									// unchanged property.
					...
					(function() {
						var fragment = document.createDocumentFragment(), div = fragment.appendChild(document.createElement("div")), input = document
								.createElement("input");
					...
					jQuery.event = {
						...
						trigger : function(event, data, elem, onlyHandlers) {
							...
							var i, cur, tmp, bubbleType, ontype, handle, special, eventPath = [ elem || document ], type = hasOwn.call(event, "type") ? event.type
									: event, namespaces = hasOwn.call(event, "namespace") ? event.namespace.split(".") : [];
						...
						dispatch : function(event) {
							...
							var i, j, ret, matched, handleObj, handlerQueue = [], args = slice.call(arguments), handlers = (data_priv.get(this, "events") || {})[event.type]
					...
					if (!support.focusinBubbles) {
						jQuery.each({
							...
							// Attach a single capturing handler on the document
					...
					jQuery.extend({
						clone : function(elem, dataAndEvents, deepDataAndEvents) {
							...
							if (!support.noCloneChecked && (elem.nodeType === 1 || elem.nodeType === 11) && !jQuery.isXMLDoc(elem)) {
								...
								// reasons: http://jsperf.com/getall-vs-sizzle/2
						...
						buildFragment : function(elems, context, scripts, selection) {
							...
							while ((elem = nodes[i++])) {
								...
								// #4087 - If origin and destination elements
								// are the same, and this is
					...
					jQuery.each({
						...
						jQuery.fn[name] = function(selector) {
							...
							for (; i <= last; i++) {
								...
								// Support: QtWebKit
								// .get() because push.apply(_, arraylike)
								// throws
					...
					(function() {
						var pixelPositionVal, boxSizingReliableVal, docElem = document.documentElement, container = document.createElement("div"), div = document
								.createElement("div");
						...
						function computePixelPositionAndBoxSizingReliable() {
							...
							"-webkit-box-sizing:border-box;-moz-box-sizing:border-box;" + "box-sizing:border-box;display:block;margin-top:1%;top:1%;"
						...
						if (window.getComputedStyle) {
							jQuery.extend(support, {
								pixelPosition : function() {
									...
									// This test is executed only once but we
									// still do memoizing
								...
								reliableMarginRight : function() {
									...
									// gets computed margin-right based on width
									// of container. (#3333)
									// WebKit Bug 13343 - getComputedStyle
									// returns wrong value for margin-right
									// This support function is only executed
									// once so no memoizing is needed.
					...
					rdisplayswap = /^(none|table(?!-c[ea]).+)/, rnumsplit = new RegExp("^(" + pnum + ")(.*)$", "i"), rrelNum = new RegExp("^([+-])=(" + pnum + ")", "i"),
					...
					function getWidthOrHeight(elem, name, extra) {
						...
						var valueIsBorderBox = true, val = name === "width" ? elem.offsetWidth : elem.offsetHeight, styles = getStyles(elem), isBorderBox = jQuery.css(
								elem, "boxSizing", false, styles) === "border-box";
					...
					jQuery.extend({
						...
						// Add in style property hooks for overriding the
						// default
						...
						style : function(elem, name, value, extra) {
							...
							if (value !== undefined) {
								...
								// If a number, add 'px' to the (except for
								...
								// Support: IE9-11+
								// background-* props affect original clone's
								// values
								...
								// otherwise just set the specified value
								if (!hooks || !("set" in hooks) || (value = hooks.set(elem, value, extra)) !== undefined) {
									style[name] = value;
								}

							} else {
								// If a hook was provided get the non-computed
								// value from there
						...
						css : function(elem, name, extra, styles) {
							...
							// If a hook was provided get the computed value
							// from there
					...
					var fxNow, timerId, rfxtypes = /^(?:toggle|show|hide)$/, rfxnum = new RegExp("^(?:([+-])=|)(" + pnum + ")([a-z%]*)$", "i"), rrun = /queueHooks$/, animationPrefilters = [ defaultPrefilter ], tweeners = {
						"*" : [ function(prop, value) {
							var tween = this.createTween(prop, value), target = tween.cur(), parts = rfxnum.exec(value), unit = parts && parts[3]
					...
					function Animation(elem, properties, options) {
						...
						return animation.progress(animation.opts.progress).done(animation.opts.done, animation.opts.complete).fail(animation.opts.fail).always(
							...
							.extend({
								...
								stop : function(type, clearQueue, gotoEnd) {
									...
									return this.each(function() {
										...
										// Start the next in the queue if the
										// last step wasn't forced.
										// Timers currently will call their
										// complete callbacks, which
										// will dequeue but only if they were
										// gotoEnd.
										if (dequeue || !gotoEnd) {
											jQuery.dequeue(this, type);
										}
								...
								finish : function(type) {
									...
									return this
											.each(function() {
												var index, data = data_priv.get(this), queue = data[type + "queue"], hooks = data[type + "queueHooks"], timers = jQuery.timers, length = queue ? queue.length
														: 0;
					...
					(function() {
						var input = document.createElement("input"), select = document.createElement("select"), opt = select
								...
								toggleClass : function(value, stateVal) {
									...
									return this.each(function() {
										if (type === "string") {
											...
											// or if we're passed `false`,
											// then remove the whole classname
											// (if there was one, the above
											// saved it).
					...
					jQuery
							.extend({
								valHooks : {
									...
									select : {
										get : function(elem) {
											var value, option, options = elem.options, index = elem.selectedIndex, one = elem.type === "select-one" || index < 0, values = one ? null
					...
					jQuery.extend({
						...
						ajaxSettings : {
							 ...
							 * null, password: null, cache: null, throws: false,
							...
							// Data converters
							// Keys separate source (or catchall "*") and
							// destination types with a single space
							converters : {
						...
						ajax : function(url, options) {
							...
							// Context for global events is callbackContext if
							...
							jqXHR = {
								...
								statusCode : function(map) {
									...
									if (map) {
										if (state < 2) {
											for (code in map) {
												// Lazy-add the new callback in
												// a way that preserves old ones
							...
							// promotion)
							// Add protocol if not provided (prefilters might
							// expect it)
							...
							// Don't fire events if jQuery.event is undefined in
							// an AMD-usage scenario (#15118)
							...
							if (!s.hasContent) {
								...
								if (s.data) {
									...
									// #9682: remove data so that it's not used
									// in an eventual retry
									delete s.data;
								}
								...
								if (s.cache === false) {
									...
									// If there is already a '_' parameter, set
									// its value
							...
							function done(status, nativeStatusText, responses, headers) {
								...
								// Convert no matter what (that way responseXXX
					...
					jQuery.fn.extend({
						...
						serializeArray : function() {
							return this.map(function() {
								// Can add propHook for "elements" to filter or
								// add form elements
								...
								return elements ? jQuery.makeArray(elements) : this;
									...
									function() {
										...
										return this.name && !jQuery(this).is(":disabled") && rsubmittable.test(this.nodeName) && !rsubmitterTypes.test(type)
					...
					jQuery.ajaxPrefilter("json jsonp", function(s, originalSettings, jqXHR) {
						...
						var callbackName, overwritten, responseContainer, jsonProp = s.jsonp !== false
								&& (rjsonp.test(s.url) ? "url" : typeof s.data === "string" && !(s.contentType || "").indexOf("application/x-www-form-urlencoded")
					...
					jQuery.fn.extend({
						offset : function(options) {
							...
							// If we don't have gBCR, just use 0,0 rather than
							// error
						...
						position : function() {
							...
							// (parentOffset = {top:0, left: 0}, because it is
							// its only offset parent
							if (jQuery.css(elem, "position") === "fixed") {
								// Assume getBoundingClientRect is there when
							...
							jQuery.fn[funcName] = function(margin, value) {
								...
								return access(this, function(elem, type, value) {
									...
									if (jQuery.isWindow(elem)) {
										...
										// incorrect results for Mobile Safari,
										// but there
										// isn't a whole lot we can do. See pull
									...
									if (elem.nodeType === 9) {
										...
										// offset[Width/Height] or
										// client[Width/Height],
										// whichever is greatest
										return Math.max(elem.body["scroll" + name], doc["scroll" + name], elem.body["offset" + name], doc["offset" + name], doc["client"
												+ name]);
									}
